...
DATA_RATIO = 0.5
GRAD_ACCUM_STEPS = 3
CKPT_PATH = "tiny_minimind_clip_unfreeze_epoch2.pt"  # ← 修改为上次保存的 checkpoint
...
class VisionToTextProj(nn.Module):
    def __init__(self, in_dim, out_dim, hidden_dim=None, num_layers=4, dropout=0.1):
        ...
        for i in range(num_layers):
            layers.append(nn.Sequential(
                nn.LayerNorm(in_dim if i == 0 else hidden_dim),  # Pre-LN
                nn.Linear(in_dim if i == 0 else hidden_dim, hidden_dim),
                nn.GELU(),
                nn.Dropout(dropout),
                nn.Linear(hidden_dim, out_dim if i == num_layers - 1 else hidden_dim)
            ))
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            residual = x
            x = layer(x)
            if i != len(self.layers) - 1:  # 最后一层不加残差
                x = x + residual
        return x
...
if __name__ == "__main__":
    ...
    if os.path.exists(CKPT_PATH):
        ...
        print(f" 成功加载 checkpoint: {CKPT_PATH} （full mode，模型+optimizer），从 epoch {start_epoch} step {global_step} 继续")
    else:
    ...
    for epoch in range(start_epoch, start_epoch + EPOCHS):
        ...
        }, f"tiny_minimind_clip_retrain_epoch{epoch}.pt")
        print(f"保存: tiny_minimind_clip_retrain_epoch{epoch}.pt")
